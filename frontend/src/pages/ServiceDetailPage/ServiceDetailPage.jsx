...
export default function ServiceDetail() {
    ...
    async function tryFetch() {
      ...
      for (const url of endpoints) {
        try {
          ...
          if (res.status === 404) { lastError = new Error(`404 ${url}`); continue; }

          const contentType = res.headers.get("content-type") || "";
          if (!res.ok || !contentType.includes("application/json")) {
            const txt = await res.text().catch(() => "");
            lastError = new Error(`Bad response ${res.status} at ${url}: ${String(txt).slice(0, 200)}`);
            continue;
          }

          const json = await res.json().catch(() => null);
          const doc  = json?.data ?? json?.service ?? json;
          if (!doc) { lastError = new Error(`No service data at ${url}`); continue; }
  ...
  function transformServiceShape(doc) {
    ...
    if (doc.slots && !Array.isArray(doc.slots) && typeof doc.slots === "object") {
      ...
      if (dates.length > 0) { dates.forEach((d) => (slotsMap[d] = arr.slice())); }
      else {
      ...
      if (dates.length > 0) { dates.forEach((d) => (slotsMap[d] = [])); }
      else {
        const today = new Date().toISOString().split("T")[0];
        dates = [today];
        slotsMap[today] = [];
  ...
  function buildPayload(extraPayment = {}) {
    return {
      serviceId:
      ...
      serviceImageUrl:
        (service?.raw &&
          (service.raw.imageUrl || service.raw.image || service.raw.imageURL || "")) ||
        service?.image || "",
  ...
  /* ── Razorpay handler ──────────────────────────────────────────────── */
  async function handleRazorpayPayment(token, headers) {
    // 1. Load Razorpay SDK
    ...
    //    Your backend should expose POST /api/payments/razorpay/create-order
    //    and return { orderId, amount, currency, keyId }
    let orderData;
    try {
      const orderRes = await fetch(`${DEFAULT_HOST}/api/payments/razorpay/create-order`, {
        method:  "POST",
        headers: { ...headers, "Content-Type": "application/json" },
        body:    JSON.stringify({
          amount:      service.price,   // in INR (backend should convert to paise)
          serviceName: service.name,
          serviceId:
            (service?.raw && (service.raw._id || service.raw.id)) || service?.id,
        }),
      ...
      if (!orderRes.ok || !orderJson?.orderId) {
        toast.error(
          (orderJson && (orderJson.message || orderJson.error)) ||
    ...
    const options = {
      ...
      handler: async (response) => {
        ...
        try {
          const confirmRes = await fetch(`${DEFAULT_HOST}/api/service-appointments`, {
            method:  "POST",
            headers: { ...headers, "Content-Type": "application/json" },
            body:    JSON.stringify(
              buildPayload({
          ...
          if (!confirmRes.ok) {
            toast.error(
              (confirmJson && (confirmJson.message || confirmJson.error)) ||
                "Payment succeeded but booking confirmation failed. Contact support.",
            );
            return;
          }

          toast.success("Payment successful! Redirecting to appointments...");
          setTimeout(() => {
          ...
          setSelectedDate(""); setSelectedTime(""); setEmail("");
        } catch (err) {
          console.error("Booking confirm error after payment:", err);
          toast.error("Payment done but failed to save booking. Contact support.");
        } finally {
          setSubmitting(false);
        }
      },

      /* ── On modal dismiss / failure ── */
      modal: {
        ondismiss: () => {
  ...
  const handleSubmit = async (e) => {
    ...
    let token = null;
    try { token = await getToken(); } catch (err) { console.error("Clerk token error:", err); }
    ...
    if (paymentMethod === "Online") {
      await handleRazorpayPayment(token, headers);
      return;
    }
    ...
    try {
      ...
      setTimeout(() => { navigate("/appointments?payment_status=Paid", { replace: true }); }, 700);
    ...
    return (
      <div className={serviceDetailStyles.loadingContainer}>
        <div className={serviceDetailStyles.loadingCard}>
          ...
          <p className={serviceDetailStyles.loadingText}>Please go back and select a valid service.</p>
          <Link to="/services" className={serviceDetailStyles.backToServices}>Back to Services</Link>
        </div>
      </div>
    );
  }
        ...
        <div className={serviceDetailStyles.rightColumn}>
          ...
          <div className={serviceDetailStyles.summaryContainer}>
            ...
            <div className={serviceDetailStyles.summaryContent}>
              ...
              <p><b>Payment:</b> {paymentMethod}{paymentMethod === "Online" ? " (Razorpay)" : ""}</p>
